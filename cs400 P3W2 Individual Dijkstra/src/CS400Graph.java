...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected Path dijkstrasShortestPath(T start, T end) throws NoSuchElementException {
    ...
    if (vertices.get(start) == null || vertices.get(end) == null) { //throwing exception if either end or start is not valid
      throw new NoSuchElementException("Path from start to end could not be found");
    }

    Vertex currentVertex = vertices.get(start);
    Path currentPath = new Path(currentVertex);
    PriorityQueue<Path> discoveredPaths = new PriorityQueue<>();
    LinkedList<Vertex> verticesWithShortestPaths = new LinkedList<>();
    LinkedList<Path> shortestPaths = new LinkedList<>();

    verticesWithShortestPaths.add(currentVertex); //adding the first vertex

    shortestPaths.add(currentPath); // Adding the initial path
    for (int i = 0; i < currentVertex.edgesLeaving.size(); i++) { // Adding the paths from the initial vertex
      if(verticesWithShortestPaths.contains(currentVertex.edgesLeaving.get(i).target) == false)    
        discoveredPaths.add(new Path(currentPath, currentVertex.edgesLeaving.get(i)));
    }

    while(discoveredPaths.isEmpty() == false) { // While there are still edges to be explored      
      currentPath = discoveredPaths.poll(); //getting the new path that is shortest and removing it from the list of discovered paths
      if(verticesWithShortestPaths.contains(currentPath.end) == false) {
        currentVertex = currentPath.end;
        verticesWithShortestPaths.add(currentVertex);
        shortestPaths.add(currentPath);

        for (int i = 0; i < currentVertex.edgesLeaving.size(); i++) { // Adding add paths from the current vertex as long as the destination has not already been found
          if(verticesWithShortestPaths.contains(currentVertex.edgesLeaving.get(i).target) == false)
            discoveredPaths.add(new Path(currentPath, currentVertex.edgesLeaving.get(i)));
        }
      }
    ...
    for (int i = 0; i < shortestPaths.size(); i++) { // find the path
      if (shortestPaths.get(i).end.data == end) {
        finalPath = shortestPaths.get(i);
      }
    }
    if(finalPath == null) // if dijkstras did not find a shortest path to the "end"
      throw new NoSuchElementException("Path from start to end could not be found");

    return finalPath; // TODO: Implement this method in Step 7.
  }
